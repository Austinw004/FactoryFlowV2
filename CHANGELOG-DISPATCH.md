...
| 06:15 | 41220d5 | F2 | F2 | F2-FILED-011 round 4 — custom ERP sentinel adapter | NEW `server/lib/erp/customAdapter.ts`. Closes the F2-FILED-011 scaffold set at 6/6 ErpSystem enum values covered. Intentionally not a working integration — returns a friendly intake message ("email sales@prescient-labs.com with: ERP name + version, API docs URL, auth mechanism, entity types, expected volume; typical timeline 2-4 weeks scoping + 6-12 weeks build") for any customer who selects "Custom" in the ERP dropdown. Better than letting the routes layer fall through to the generic "in beta" copy because (a) tells routes "yes, we know this system; per-engagement build needed", (b) gives a single intake message to tune in one place, (c) matches the response shape of the other 5 adapters for consistent frontend UX. | pushed; awaiting Republish |
| 06:00 | 86f13b2 | doc | F2+F1 | F2-FILED-008 + F1-FILED-009 + F1-FILED-010 marked RESOLVED — ledger audit caught 3 fixes that shipped without being recorded | All three items audited and found to be FIXED in the current code, but never updated in the dispatch ledger. F2-FILED-008 placeholder filter present at `server/routes.ts:10667+10706`. F1-FILED-009 UNKNOWN-regime fallback present at `server/lib/copilotService.ts:172-184` (falls back to `DualCircuitEconomics.getCurrentRegime()` when per-company lookup misses). F1-FILED-010 structural fix present at `server/lib/smartInsights.ts:182-243` (reads materials table with tenant-aware bottom-quartile threshold instead of non-existent SKU stock columns). All three changelog sections updated with the proof-of-fix references. **All open F1s now closed; all open F2s either resolved or in shipping-progress (F2-FILED-011 has 5/6 scaffolds shipped).** | doc-only, no code change |
| 05:30 | 88d9e88 | F2 | F2 | F2-FILED-011 round 3 — Oracle Fusion + Acumatica adapter scaffolds | NEW `server/lib/erp/oracleAdapter.ts`: env-gated on ORACLE_INSTANCE_URL + EITHER (ORACLE_USERNAME + ORACLE_PASSWORD basic auth) OR (ORACLE_OAUTH_TOKEN_URL + ORACLE_CLIENT_ID + ORACLE_CLIENT_SECRET + ORACLE_SCOPE IDCS OAuth2). Auto-selects auth flavor based on env vars present. testConnection probes `/fscmRestApi/resources/11.13.18.05/suppliers?limit=1`. 404 error includes specific diagnostic about REST API version mismatch (scaffold targets 11.13.18.05 / Fusion release 23B+). NEW `server/lib/erp/acumaticaAdapter.ts`: env-gated on ACUMATICA_BASE_URL + USERNAME + PASSWORD + COMPANY (plus optional TENANT, ENDPOINT_VERSION). Session-cookie auth (POST /entity/auth/login → cache cookies → use on reads); auto-clear cookie cache on 401 for fresh re-login. testConnection probes Vendor entity. 404 error documents the endpoint-version naming convention (Default/22.200.001 → Default/23.200.001 for newer builds). Both adapters: full error class handling with diagnostic-quality reasons pointing at the most common misconfiguration modes. List-entity stubs include exact REST URLs + Acumatica's `{ value: ... }` wrapping note for whoever wires the real path. Updated `erp/index.ts` to import all 5 adapters at boot. **Five of six ErpSystem enum values now scaffolded** (NetSuite, SAP, Dynamics, Oracle, Acumatica) — only "custom" remains, which by definition has no adapter (customer-built ERPs need per-engagement integrations). | pushed; awaiting Republish |
...
### F2-FILED-008 — Commodity forecasts surface placeholder material names — **RESOLVED**

- **Original symptom**: `/api/commodity-forecasts` for a seeded tenant returned 10 rows including 4 placeholders (`RAW-MATERIAL-001`, `COMPONENTS-002`, `PACKAGING-003`, `CONSUMABLES-004`) alongside the customer's real Steel/Al/Cu materials. Read as demo content leaking into the real product.
- **Fix in code** (round-22 audit found it already shipped, never recorded in ledger): `server/routes.ts:10667-10669` (in `/api/commodity-forecasts`) AND `server/routes.ts:10706-10708` (in `/api/commodity-forecasts/summary`) both apply `m => !/^(RAW-MATERIAL|COMPONENTS|PACKAGING|CONSUMABLES)-\d+$/.test(m.code)` to filter placeholder material codes BEFORE passing to `generateCommodityForecasts`. Tenant sees only their real materials. Demo tenants with intentional placeholder data already get filtered too (by design — those names aren't real commodities and don't have pricing mappings).
- **Verification**: comment block in both handlers explicitly explains the filter purpose and references the original symptom.

### F1-FILED-009 — AI Advisor pushback doesn't fire for tenants whose regime lookup returns "Unknown" — **RESOLVED**

- **Original symptom**: regime-coherence hostile-prompt pushback (from `fd76178`) only fired when `activeRegime` was one of the four canonical regimes. Legacy tenants without an `economicSnapshots` row got `activeRegime = "UNKNOWN"` → no pushback even on clearly hostile prompts.
- **Fix in code** (round-22 audit found it already shipped, never recorded in ledger): `server/lib/copilotService.ts:172-184` implements exactly the proposed fallback. When the per-company `getCompanyRegimeIntelligence` lookup returns UNKNOWN, the code falls back to `DualCircuitEconomics.getCurrentRegime()` to pull the global economic regime. If THAT returns one of the canonical four (HEALTHY_EXPANSION, IMBALANCED_DEFICIT, etc.), `activeRegime` is bumped to it before the hostile-intent detector runs.
- **Verification**: comment in the source ("Either is failure-tolerant: a regime lookup miss should never block the user response, and the worst case is that posture defaults to HEALTHY_EXPANSION below") documents the fallback chain. Legacy tenants now get full regime-aware pushback against hostile prompts using the global regime as their reference frame.

### F1-FILED-010 — Smart Insights uses non-existent SKU fields (structural) — **RESOLVED**

- **Original symptom**: `getSkuInsights()` filtered SKUs by `s.currentStock <= s.safetyStock * 1.2` but the `skus` table only has `{id, companyId, code, name, priority, createdAt}` — no stock or safetyStock columns existed. Filter evaluated to `0 <= 0 * 1.2 = true` for every SKU → every tenant saw "N Products Need Attention" flagging ALL their SKUs regardless of actual inventory.
- **Fix in code** (round-22 audit found it already shipped, never recorded in ledger): `server/lib/smartInsights.ts:182-243` was rewritten. Now it reads the `materials` table (where real `onHand` quantities live, populated by the seed/import flow) instead of the SKU rows. Computes a tenant-aware low-stock threshold dynamically: bottom-quartile `onHand` across all the tenant's materials, with a floor of 100 units so single-material tenants don't get false positives on healthy stock. Projects materials into the shape callers expect (`{name, currentStock, onHand, unit}`). `high-demand` filter returns empty until demand-history aggregation is wired up (rather than emit false positives from non-existent fields).
- **Verification**: dedicated comment block in the source explains both the original structural bug ("filter evaluated 0 <= 0 * 1.2 for every SKU") and the rationale for the new approach (tenant-aware threshold, materials table as source-of-truth). End result: low-stock alerts now reflect actual inventory state, not a phantom comparison against missing columns.

### F2-FILED-011 — No real server-side ERP adapter behind /api/erp-connections/test — **SCAFFOLDS SHIPPED** (round-18: NetSuite; round-19: SAP + Dynamics 365; round-21: Oracle + Acumatica)

- **Severity** F2 — exposed during the F1-FILED-004 fix (`a4315a1`). The test endpoint now dispatches via a real adapter registry; **five of six** ErpSystem enum values are shipped as scaffolds (NetSuite, SAP S/4HANA Cloud, Dynamics 365 Business Central, Oracle Fusion Cloud ERP, Acumatica Cloud ERP). Only "custom" remains — and by definition there's no adapter for an unspecified customer-built ERP; customers running custom ERPs typically need a per-engagement integration build. The routes layer falls back to the honest "connector in beta — contact sales" message for systems without an adapter or without credentials.

**Round-21 Oracle + Acumatica additions (companion to round-18 NetSuite + round-19 SAP/Dynamics):**

- **`server/lib/erp/oracleAdapter.ts`** (NEW) — `OracleAdapter` class. Supports BOTH common Fusion auth flavors: Basic Auth (sandboxes + smaller production tenants without IDCS federation) and IDCS OAuth2 client_credentials (enterprise tenants). Adapter auto-selects flavor based on env vars present: if `ORACLE_OAUTH_TOKEN_URL + ORACLE_CLIENT_ID + ORACLE_CLIENT_SECRET + ORACLE_SCOPE` are all set, uses OAuth2; otherwise falls to `ORACLE_USERNAME + ORACLE_PASSWORD` basic auth. `testConnection` probes `/fscmRestApi/resources/11.13.18.05/suppliers?limit=1`. Includes specific 404 diagnostic about REST API version mismatch (scaffold version 11.13.18.05 corresponds to Fusion release 23B+; older tenants need a different version segment).

- **`server/lib/erp/acumaticaAdapter.ts`** (NEW) — `AcumaticaAdapter` class. Session-cookie auth (NOT bearer token — that's Acumatica's contract): POST `/entity/auth/login` with `{name, password, company, tenant?}`, parse Set-Cookie response into Cookie: header for subsequent calls. Cookie cached for process lifetime; auto-clear on 401 triggers fresh re-login. `testConnection` probes `Vendor` entity. 404 diagnostic documents the endpoint-version naming convention (Default/22.200.001 for 2022 R2 builds, Default/23.200.001 for newer). Optional `ACUMATICA_TENANT` env var supports multi-tenant deployments.

- **`server/lib/erp/index.ts`** updated — all 5 adapters now imported for side-effect registration at server boot. Every adapter short-circuits to `not_configured` until its env vars are present, so importing all five is safe even when only one (or none) is provisioned.
