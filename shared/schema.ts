...
// Routine digests — the meta-routine compiles results from all six reporting
// background jobs (economic data, sensor readings, commodity prices, ML
// predictions, supply-chain risk, workforce metrics) into a single email
// digest with confirm/reject links per proposed change. Tokens are signed
// links so confirmations can happen without re-auth from the email client.
export const routineDigests = pgTable(
  "routine_digests",
  {
    id: varchar("id")
      .primaryKey()
    ...
    // Per-routine result: { name, status: 'ok' | 'error' | 'skipped',
    // durationMs, summary, error }
    routineResults: jsonb("routine_results").notNull(),
    // Each pending change the digest surfaces for human review.
    // { id, routineName, kind, title, detail, status: 'pending' | 'approved'
    //   | 'rejected', resolvedAt }
    proposedChanges: jsonb("proposed_changes").notNull(),
    // Signed token used to authenticate confirm/reject links from the email.
    confirmToken: varchar("confirm_token").notNull().unique(),
    // Free-form prompt the user replied with via the digest's prompt-back
    // endpoint (web form or future inbound-email parser).
    userPrompt: text("user_prompt"),
    promptRespondedAt: timestamp("prompt_responded_at"),
  ...
  (table) => [
    index("idx_routine_digests_user").on(table.userId),
    index("idx_routine_digests_run_at").on(table.runAt),
    uniqueIndex("idx_routine_digests_token").on(table.confirmToken),
  ],
);
export const insertRoutineDigestSchema = createInsertSchema(routineDigests).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
export type RoutineDigest = typeof routineDigests.$inferSelect;
export type InsertRoutineDigest = z.infer<typeof insertRoutineDigestSchema>;

// Result of a single sub-routine run within a digest.
...
// A change the digest proposes for human approval before applying.
// `applyPayload` is the executable action the registered handler for `kind`
// runs when the user clicks Approve from the email. `applyResult` records
// what happened so it shows up in admin history; `appliedAt` is the apply
// timestamp (distinct from `resolvedAt`, which is just the user's click).
export interface ProposedChange {
  id: string;
  routineName: string;
  kind: string; // e.g. "regime-shift", "price-alert", "model-retrain"
  title: string;
  detail: string;
