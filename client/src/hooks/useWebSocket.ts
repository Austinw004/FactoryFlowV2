    ...
    try {
      ...
      ws.onmessage = (event) => {
        ...
        if (message.type === 'connection_established') {
          if (message.message && import.meta.env.DEV) console.log('[WebSocket]', message.message);
          return;
        }
      ...
      ws.onclose = () => {
        ...
        if (reconnectAttemptsRef.current < MAX_RECONNECT_ATTEMPTS) {
          ...
          console.warn('[WebSocket] Max reconnect attempts reached. Real-time updates paused.');
        }
      };
    } catch (error) {
