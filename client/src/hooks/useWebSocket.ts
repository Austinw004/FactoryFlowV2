    ...
    try {
      ...
      ws.onmessage = (event) => {
        ...
        if (message.type === 'connection_established') {
          if (IS_DEV && message.message) console.log('[WebSocket]', message.message);
          return;
        }
      ...
      ws.onclose = () => {
        ...
        if (reconnectAttemptsRef.current < MAX_RECONNECT_ATTEMPTS) {
          ...
          if (IS_DEV) console.log(`[WebSocket] Disconnected. Reconnecting in ${jittered}ms (attempt ${reconnectAttemptsRef.current + 1}/${MAX_RECONNECT_ATTEMPTS})`);
          ...
          if (IS_DEV) console.log('[WebSocket] Max reconnect attempts reached. Real-time updates paused.');
        }
      };
    } catch (error) {
