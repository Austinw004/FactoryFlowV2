    ...
    try {
      ...
      ws.onmessage = (event) => {
        ...
        if (message.type === 'connection_established') {
          if (message.message) wsLog('[WebSocket]', message.message);
          return;
        }
      ...
      ws.onclose = () => {
        ...
        if (reconnectAttemptsRef.current < MAX_RECONNECT_ATTEMPTS) {
          ...
          wsLog(`[WebSocket] Disconnected. Reconnecting in ${jittered}ms (attempt ${reconnectAttemptsRef.current + 1}/${MAX_RECONNECT_ATTEMPTS})`);
          ...
          wsLog('[WebSocket] Max reconnect attempts reached. Real-time updates paused.');
        }
      };
    } catch (error) {
