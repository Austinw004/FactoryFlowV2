...
**Version:** 1.2 · **Last updated:** 2026-05-15 · **Maintainer:** Austin Wendler
...
### 3.1 Access Control
- **3.1.1** Limit system access to authorized users — ✅ `isAuthenticated` middleware; 725 of 738 routes gated. The 13 ungated routes are intentionally public: 3 webhook receivers (cryptographic signature verification), and 10 unauthenticated endpoints (login, signup, password reset, healthcheck).
...
- **3.14.2** Protect against malicious code — ✅ helmet headers (CSP frame-ancestors for clickjacking, X-Content-Type-Options, Referrer-Policy, Permissions-Policy, COOP same-origin, HSTS preload-eligible in prod), `x-powered-by` removed, Zod input validation at every route boundary, bulk endpoints bounded at 1000 items.
