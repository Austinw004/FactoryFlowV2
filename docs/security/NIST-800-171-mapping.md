...
**Version:** 1.2 · **Last updated:** 2026-05-08 · **Maintainer:** Austin Wendler
...
### 3.1 Access Control
- **3.1.1** Limit system access to authorized users — ✅ `isAuthenticated` middleware; 731 of 739 routes gated. The 8 public routes are `/healthz`, `/readyz`, `/livez`, `/api/stripe/config`, `/api/stripe/products`, `/api/status/summary`, `/api/status/history`, and `/api/contact-sales` (rate-limited, honeypotted).
...
- **3.14.2** Protect against malicious code — ✅ helmet headers (X-Content-Type-Options, Referrer-Policy, Permissions-Policy, HSTS preload, CSP with `frame-ancestors` for clickjacking — X-Frame-Options is intentionally omitted because Replit Preview iframes from a different subdomain), `x-powered-by` removed, input validation at every route boundary.
