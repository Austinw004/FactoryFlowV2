...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/rfqs", isAuthenticated, rateLimiters.api, async (req: any, res) => {
    try {
      ...
      // The full insertRfqSchema requires companyId / rfqNumber / regime
      // context, all of which are server-derived. Validating the raw client
      // body against it 400'd every manual-create attempt (round-10 live
      // audit caught this). Strip the server-injected fields out of the
      // client-input schema; the handler injects them below.
      const clientRfqSchema = insertRfqSchema.omit({
        companyId: true,
        createdBy: true,
        isAutoGenerated: true,
        rfqNumber: true,
        regimeAtGeneration: true,
        fdrAtGeneration: true,
        policySignal: true,
        inventoryLevelAtTrigger: true,
        reorderPointThreshold: true,
        triggerReason: true,
      });
      const validationResult = clientRfqSchema.safeParse(req.body);
      if (!validationResult.success) {
        return res.status(400).json({
      ...
      // Derive rfqNumber server-side (RFQ-YYYY-NNNN, scoped to company).
      // Mirrors the auto-generation flow in server/lib/rfqGeneration.ts.
      const year = new Date().getFullYear();
      const existingRfqsThisYear = await db
        .select({ rfqNumber: rfqs.rfqNumber })
        .from(rfqs)
        .where(
          and(
            eq(rfqs.companyId, user.companyId),
            sql`${rfqs.rfqNumber} LIKE ${`RFQ-${year}-%`}`,
          ),
        );
      const rfqNumber = `RFQ-${year}-${String(existingRfqsThisYear.length + 1).padStart(4, "0")}`;

      // Capture regime context at creation time from the latest snapshot.
      // The schema requires non-null values for these; fall back to
      // HEALTHY_EXPANSION / FDR 1.0 if no snapshot exists yet for the
      // tenant (early in their lifecycle, before the regime calc job runs).
      const snapshot = await storage.getLatestEconomicSnapshot(user.companyId);
      const regimeAtGeneration = snapshot?.regime ?? "HEALTHY_EXPANSION";
      const fdrAtGeneration = snapshot?.fdr ?? 1.0;
