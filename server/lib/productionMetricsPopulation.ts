    ...
    for (const machine of companyMachinery) {
      ...
      for (const run of runs) {
        ...
        try {
          ...
          console.warn(`[ProductionKPI] Failed to insert metric for run:`, error);
        }
      }
      ...
      if (events.length > 3) {
        ...
        for (const bottleneck of bottlenecks) {
          try {
            ...
            console.warn(`[ProductionKPI] Failed to insert bottleneck:`, error);
          }
        }
      }
    }
  ...
  private async generateSyntheticMetrics(companyId: string): Promise<ProductionMetricsResult> {
    ...
    for (const machine of syntheticMachinery) {
      ...
      try {
        ...
        console.warn(`[ProductionKPI] Failed to insert synthetic metric for "${machine.name}":`, error);
      }
    }
