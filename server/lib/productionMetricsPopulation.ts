    ...
    for (const machine of companyMachinery) {
      ...
      for (const run of runs) {
        ...
        try {
          ...
          console.error(`[ProductionKPI] Failed to insert production metric for run ${run.id ?? "unknown"}:`, error);
        }
      }
      ...
      if (events.length > 3) {
        ...
        for (const bottleneck of bottlenecks) {
          try {
            ...
            console.error(`[ProductionKPI] Failed to insert bottleneck for machine ${machine.id ?? "unknown"}:`, error);
          }
        }
      }
    }
  ...
  private async generateSyntheticMetrics(companyId: string): Promise<ProductionMetricsResult> {
    ...
    for (const machine of syntheticMachinery) {
      ...
      try {
        ...
        console.error(`[ProductionKPI] Failed to insert synthetic metric for "${machine.name}":`, error);
      }
    }
