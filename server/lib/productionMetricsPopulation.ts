    ...
    for (const machine of companyMachinery) {
      ...
      for (const run of runs) {
        ...
        try {
          ...
          console.warn('[ProductionKPI] metric insert skipped:', (error as Error).message);
        }
      }
      ...
      if (events.length > 3) {
        ...
        for (const bottleneck of bottlenecks) {
          try {
            ...
            console.warn('[ProductionKPI] bottleneck insert skipped:', (error as Error).message);
          }
        }
      }
    }
  ...
  private async generateSyntheticMetrics(companyId: string): Promise<ProductionMetricsResult> {
    ...
    for (const machine of syntheticMachinery) {
      ...
      try {
        ...
        console.warn('[ProductionKPI] synthetic metric insert skipped:', (error as Error).message);
      }
    }
